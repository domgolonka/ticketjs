!(function(window, $, undefined) {
    ...
    var smb = {
        ...
        init: function() {
            ...
            if (/^\/pages\/.*\/edit/.test(window.location.pathname) ||
                ...
                $("#editor").closest("form").on("submit", function () {
                    ...
                    window.sessionStorage.setItem("smb-page", $("#editor").val());
                });
                if (window.location.search == "?e=1") {
                    // Edit page in error: restore the body
                    var content;
                    if (content = window.sessionStorage.getItem("smb-page")) {
                        $("#editor").val(content);
                    }
                }
                else {
                    window.sessionStorage.removeItem("smb-page");
                }
            }
        ...
        toggleFullscreen: function() {
            ...
            var isFullscreen = smb.cmInstance.getOption("fullScreen");

            smb.cmInstance.setOption("fullScreen", !smb.cmInstance.getOption("fullScreen"));
            smb.cmInstance.focus();
...
smb.cmInstance = CodeMirror.fromTextArea(document.getElementById("editor"), {
    lineNumbers: true,
    mode: 'markdown',
    matchBrackets: true,
    lineWrapping: true,
